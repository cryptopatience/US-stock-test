...
def get_top_30_tickers():
    """🟢 변경됨: 전체 500개 스캔 후 시총 상위 30개 추출"""
    
    # 1. 위키피디아에서 500개 종목 가져오기
    ...
    status_text.text(f"S&P 500 전체 {len(sp500_tickers)}개 시가총액 스캔 중...")

    # 2. 빠른 처리를 위한 배치(Batch) 작업 (50개씩)
    batch_size = 50
    chunks = [sp500_tickers[i:i + batch_size] for i in range(0, len(sp500_tickers), batch_size)]
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            for ticker in chunk:
                try:
                    # 🟢 핵심: .info 대신 .fast_info 사용 (속도 10배 향상)
                    # 전체 500개의 상세 정보(info)를 다 받으면 너무 느리므로, 
                    # 시가총액(market_cap)만 빠르게 확인합니다.
                    t_obj = tickers_obj.tickers[ticker]
                    m_cap = t_obj.fast_info['market_cap']
                    
                    if m_cap is not None:
                        market_caps.append({'Ticker': ticker, 'Market_Cap': m_cap})
                except:
                    continue
        except:
            continue
    ...
    # 3. 시가총액 순으로 정렬하여 상위 30개 자르기
    df_caps = pd.DataFrame(market_caps)
    df_top30 = df_caps.sort_values('Market_Cap', ascending=False).head(30)
    
    # 4. 선정된 상위 30개에 대해서만 상세 정보(회사명, 섹터) 요청
    status_text.text("상위 30개 종목 상세 정보 로딩 중...")
    final_data = []
    
    for idx, row in df_top30.iterrows():
        try:
            ticker = row['Ticker']
            # 상위 30개는 개수가 적으므로 info를 호출해도 괜찮음
            ...
            final_data.append(row) # 실패 시 기본 정보만
